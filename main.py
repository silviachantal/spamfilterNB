...
def create_parser():
    ...
    parser.add_argument('-ud', '--diremails_classify', metavar='dataset path', required='True', type=str, help='enter a path to a dataset of emails that you want to classify' )
    args = parser.parse_args()
    return args

def main():
    arguments = create_parser() 
    read_train = corpus.read_dataset(arguments.train_dataset)
    read_test = corpus.read_dataset(arguments.test_dataset)
    read_usermails = corpus.read_dataset(arguments.resultfile)
    spam_filter = SpamFilter()
    spam_filter.train(read_train)
    
    with open(arguments.resultfile, "w") as final_file:
